...
export const columns: ColumnDef<ApiKey>[] = [
    ...
    },
    {
        ...
        header: "Prefix",
        cell: ({ row }) => <code className="text-xs bg-muted px-1.5 py-0.5 rounded font-mono">{row.getValue("key_prefix")}...</code>,
    },
    {
        accessorKey: "scopes",
        header: "Scopes",
        cell: ({ row }) => {
            ...
            return <div className="text-xs text-muted-foreground max-w-[200px] truncate" title={scopes.join(", ")}>{scopes.join(", ")}</div>
        },
    },
    {
        accessorKey: "id",
        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-2">
                <code className="relative rounded bg-muted/50 px-[0.3rem] py-[0.2rem] font-mono text-[10px] text-muted-foreground truncate max-w-[100px]" title={row.getValue("id")}>
                    {row.getValue("id")}
                </code>
            </div>
        ),
